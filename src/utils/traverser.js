export default function traverserHead(rootEl, bar) {
  const parsedEl = toJSON(rootEl).childNodes[0];

  return traverser(parsedEl, bar);
}

function traverser(parsedObj, bar) {
  const out = { id: "", name: "", childs: [] };

  // recursive searching
  parsedObj.childNodes.forEach((el) => {
    if (el.tagName === "button") {
      el.attributes.forEach((attr) => {
        if (attr[0] === "id") {
          out.id = attr[1];
          out.name = el.childNodes[0].nodeValue;
        }
      });
    } else if (el.tagName === "div") {
      out.childs.push(traverser(el, bar));
    }
  });

  // adding additional properties
  if (out.childs.length === 0) out.childs = null;
  out[bar] = out.childs ? out.childs.length : 1;
